<template>
    ...
    <!-- 一开始数据还没有过来，先判断有没有拿到数据，拿到数据才显示 -->
    <div class="content" v-if="detail.detail">  
      <!-- 动态绑定src，读取用户头像 baseurl是后台数据库端口 -->
      <div class="header clear"><h2><img :src="`${baseurl}${detail.detail.auth_icon}`" alt=""/></h2><p>{{detail.detail.auth}}</p></div>
      <div class="cont">
        <h3>{{detail.title}}</h3>
...
export default {
  ...
  // },
    ...
    mounted(){
      // console.log("detail",this.baseurl)
      ...
      // })
      // console.log('detail',dataName,id)
      this.$store.dispatch({
        type:'View_detail',
        payload:{dataName,id}
      })
